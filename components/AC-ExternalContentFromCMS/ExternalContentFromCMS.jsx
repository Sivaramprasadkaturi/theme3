  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      if (LocationsEnum[place] === "supplier-product-banners?VID") {
        ...
        console.log(
          `${process.env.NEXT_AVETTI_CMS}/${LocationsEnum[place]}=${
            location.origin
          }${
            location.pathname.substring(0, 8) +
            location.pathname.substring(8).replace("/", "")
          }`
        );
        return await fetch(
          `${process.env.NEXT_AVETTI_CMS}/${LocationsEnum[place]}=${
        ...
        <>
          {remoteDataStrapi.data?.ContentEntries?.filter(
            ...
            >
              ...
              {el.Images &&
                el.Description &&
                el.Description.Position === "Text_Over_Image" && (
                  <>
